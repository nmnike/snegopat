...
:dlls user32.dll
	...
	stdcall BOOL IsWindow(HWND hWnd)|IsWindow
...
:dlls kernel32.dll
	...
	stdcall DWORD GetFileAttributes(int_ptr path)|GetFileAttributesW
	stdcall HANDLE FindFirstFile(int_ptr path, int_ptr lpFindFileData)|FindFirstFileW
	stdcall BOOL FindNextFile(HANDLE hFindFile, int_ptr lpFindFileData)|FindNextFileW
	stdcall BOOL FindClose(HANDLE hFindFile)|FindClose
	stdcall HANDLE CreateFile(int_ptr lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, int_ptr lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)|CreateFileW
	stdcall DWORD GetFileSize(HANDLE hFile, int_ptr lpFileSizeHigh = 0)|GetFileSize
	stdcall BOOL ReadFile(HANDLE hFile, int_ptr lpBuffer, DWORD nNumberOfBytesToRead, int_ptr lpNumberOfBytesRead = 0, int_ptr lpOverlapped = 0)|ReadFile
	stdcall BOOL WriteFile(HANDLE hFile, int_ptr lpBuffer, DWORD nNumberOfBytesToWrite, int_ptr lpNumberOfBytesWritten = 0, int_ptr lpOverlapped = 0)|WriteFile
	stdcall BOOL CloseHandle(HANDLE hObject)|CloseHandle
	stdcall void ExitProcess(uint uExitCode)|ExitProcess
	stdcall BOOL TerminateProcess(HANDLE hProcess, UINT uExitCode)|TerminateProcess
	stdcall HANDLE GetCurrentProcess()|GetCurrentProcess
